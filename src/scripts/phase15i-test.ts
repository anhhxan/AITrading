...
async function runTests() {
  ...
  const apiUrl = `http://localhost:3000/api/robots/${robotId}/test-signal`;

  console.log(`[TEST RUNNER] Starting single test against ${apiUrl}...`);
  try {
      ...
      if (data.supabase_persistence === 'SUCCESS') {
          ...
          for (let i = 0; i < 10; i++) {
              const res2 = await fetch(apiUrl, { method: 'POST' });
      ...
      console.error(`Failed to reach API. Is 'npm run dev' running?`, err);
  }
}
