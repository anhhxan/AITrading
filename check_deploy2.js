...
async function run() {
    const { data: cmds } = await supabase.from('robot_commands')
        .select('command_id, created_at, processing_started_at, processed_at, status, worker_id, command_type')
    ...
    const { data: ea43 } = await supabase.from('robot_commands')
        .select('command_id, created_at, processing_started_at, processed_at, status, worker_id')
