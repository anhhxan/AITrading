...
async function runAudit() {
  ...
  const robots = allRobots ? allRobots.filter(r => (r.mode === 'PAPER' || r.mode === 'paper') && r.status === 'ACTIVE') : [];
  ...
  if (cErr) console.error("Error fetching commands:", cErr);
    ...
    console.log(`Total commands for paper robots since 20:00 yesterday: ${paperCommands.length}`);
    paperCommands.forEach(c => {
      console.log(`CMD: ${c.id} | Robot: ${c.robot_id} | Payload: ${JSON.stringify(c.payload)} | Status: ${c.status} | Created: ${c.created_at} | Claimed: ${c.claimed_at} | Processed: ${c.processed_at}`);
    });
  }
  ...
  if (eErr) console.error("Error fetching events:", eErr);
    ...
    paperEvents.forEach(e => {
       console.log(`Event: ${e.event_type} | State: ${e.state} | Reason: ${e.reason} | Signal: ${e.signal_direction} | Robot: ${e.robot_id} | Created: ${e.created_at}`);
    });
  }
  ...
  const paperIntents = (intents || []).filter(i => robotIds.includes(i.robot_id));
  console.log(`Intents for paper robots: ${paperIntents.length}`);
  
  const { data: activeOrders } = await supabase.from('active_orders').select('*').gte('created_at', yesterday20h);
  const paperOrders = (activeOrders || []).filter(o => robotIds.includes(o.robot_id));
  console.log(`Orders for paper robots: ${paperOrders.length}`);
  
  console.log("\n=== ALL ROBOT COMMANDS RAW ===");
  if (commands) console.log(`Total commands overall (any robot): ${commands.length}`);
}
