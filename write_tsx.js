...
const fileContent = \import { createClient } from '@/lib/supabase/server'
...
function formatTimeAgo(dateString: string) {
  ...
  if (diffInSeconds < 3600) return \\\\\\ minutes ago\\\
...
export default async function DashboardOverview() {
  ...
  if (!commandsError && lastCommands && lastCommands.length > 0) {
    ...
    lastCommandActivity = \\\Last processed \\\\\\
              ...
              <tbody className="divide-y divide-slate-100">
                {lastCommands.map((cmd) => {
                  ...
                  if (result && typeof result === 'object' && result.status) {
                     ...
                     if (result.reason) execResultStr += \\\: \\\\\\
                  ...
                  return (
                    <tr key={cmd.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className={\\\px-6 py-4 \\\\\\}>{side}</td>
...
\
fs.writeFileSync('src/app/dashboard/page.tsx', fileContent, 'utf8')
