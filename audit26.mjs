...
async function check() {
    const { data: traces } = await supabase.from('signal_trace_events').select('*').eq('robot_id', robotId).order('bar_timestamp', { ascending: false }).limit(2);
    console.log(traces.map(t => ({ ts: new Date(Number(t.bar_timestamp)).toISOString(), p: t.poller_status, a: t.adapter_status, s: t.strategy_status })));
}
check();
