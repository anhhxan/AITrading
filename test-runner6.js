...
async function run() { 
  ...
  const { data: config, error: err2 } = await supabase.from('robot_configs').insert({ 
    ...
    risk_profile: { max_position_size: 100, position_allocation_percent: 10, stop_loss_pct: 2.0, leverage: 10 }, 
  ...
  console.log('6. Sending Webhook 2 (Trigger LONG SIGNAL)...'); 
  const payload2 = { 
    action: 'BUY', 
    tvSymbol: 'BINANCE:BTCUSDT.P', 
    timeframe: '15m',
  ...
  console.log('6.5 Sending Webhook 3 (Retracement Candle)...');
  const payload3 = {
    action: 'BUY', 
    tvSymbol: 'BINANCE:BTCUSDT.P', 
    timeframe: '15m',
  ...
  const { data: cmds } = await supabase.from('robot_commands').select('status, result').eq('robot_id', robot.id).is('parent_command_id', null).order('created_at', { ascending: false }).limit(3); 
  console.log('Last 3 Commands:', cmds.map(c => c.status)); 
  ...
  console.log('8. Testing REVERSAL (Trigger SHORT SIGNAL)...');
  const payload4 = { 
    action: 'SELL', 
    tvSymbol: 'BINANCE:BTCUSDT.P', 
    timeframe: '15m',
  ...
  const payload5 = { 
    ...
    tvSymbol: 'BINANCE:BTCUSDT.P', 
    timeframe: '15m',
    barTimestamp: Date.now() + 45 * 60000,
    open: 99000,
    high: 99000,
    low: 95000,
  ...
  let res5 = await fetch(url, { method: 'POST', body: JSON.stringify(payload5) }); 
  console.log('Webhook 5 status:', res5.status); 
  await new Promise(r => setTimeout(r, 6000)); 
  ...
  const payload6 = {
    ...
    tvSymbol: 'BINANCE:BTCUSDT.P', 
    timeframe: '15m',
    barTimestamp: Date.now() + 60 * 60000,
    open: 97000,
    high: 99800, // hits trigger zone (upper: 100000, lower: 99600) for short
    low: 95000,
    close: 96000,
    volume: 10,
    indicator: { length: 20, source: 'close', mult: 2.0, mult2: 1.0 },
  ...
  let res6 = await fetch(url, { method: 'POST', body: JSON.stringify(payload6) });
  console.log('Webhook 6 status:', res6.status);
  await new Promise(r => setTimeout(r, 6000));
